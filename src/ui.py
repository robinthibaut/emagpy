...
def errorMessage(etype, value, tb):
    ...
    msg.setInformativeText(
        """Please see the detailed error below.<br>You can report the errors at:<p><a href='https://gitlab.com/hkex/emagpy/issues'>https://gitlab.com/hkex/emagpy/issues</a></p><br>"""
    )
...
class MatplotlibWidget(QWidget):
    ...
    def __init__(self, parent=None, navi=True, itight=True, threed=False):
        super(MatplotlibWidget, self).__init__(
            parent
        )  # we can pass a figure but we can replot on it when
...
class App(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.errorLabel = QLabel(
            '<i style="color:black">Error messages will be displayed here</i>'
        )
        ...
        def fimportBtnFunc():
            ...
            fname, _ = self._dialog.getOpenFileName(
                importTab, "Select data file", self.lastdir, "*.csv"
            )
            if fname != "":
        ...
        self.fimportBtn.setToolTip(
            "File needs to be .csv with layer1, layer2, depth1, ... columns.\n"
        ...
        class ParamTable(QTableWidget):
            ...
            def setLayer(self, nlayer):
                ...
                for i in range(nlayer - 1):
                    depth = (i + 1) * 0.5
                    self.setItem(
                        nlayer + i, 0, QTableWidgetItem("depth{:d}".format(i + 1))
                    )
                    self.item(nlayer + i, 0).setFlags(Qt.ItemIsEnabled)
                    self.setItem(
                        nlayer + i, 1, QTableWidgetItem("{:.1f}".format(depth))
                    )
                    self.setItem(
                        nlayer + i, 2, QTableWidgetItem("{:.1f}".format(depth))
                    )
        ...
        coilLabel = QLabel(
            "Specify coil orientation (HCP or VCP), coil separation [m],"
        ...
        def fforwardBtnFunc():
            ...
            self.writeLog(
                'k.forward("{:s}", coils={:s}, noise={:.2f}'.format(
        ...
        sensors = ["CMD Mini-Explorer", "CMD Explorer", "CMD Mini-Explorer 6L"]
        ...
        def importBtnFunc():
            self._dialog = QFileDialog()
            fnames, _ = self._dialog.getOpenFileNames(
                importTab, "Select data file(s)", self.lastdir, "*.csv *.CSV"
            )
            if len(fnames) > 0:
        ...
        def importGFLoFunc():
            fname, _ = QFileDialog.getOpenFileName(
                importTab, "Select data file", self.lastdir, "*.dat *.DAT"
            )
            if fname != "":
                self.lastdir = os.path.dirname(fname)
                self.fnameLo = fname
        ...
        def importGFApplyFunc():
            ...
            if self.fnameLo is not None and self.fnameHi is not None:
                self.writeLog(
                    'k.importGF("{:s}", "{:s}", "device={:s}", hx={:.2f})'.format(
                        self.fnameLo, self.fnameHi, device, hx
                    )
                )
            if self.fnameLo is not None:
                self.writeLog(
                    'k.importGF("{:s}", "device={:s}", hx={:.2f})'.format(
        ...
        def gfCorrectionBtnFunc():
            ...
            self.writeLog(
                'k.gfCorrection(calib="{:s}")'.format(self.gfCalibCombo.currentText())
            )
            self.replot()

        self.gfCorrectionBtn = QPushButton("Convert to LIN ECa")
        self.gfCorrectionBtn.clicked.connect(gfCorrectionBtnFunc)
        self.gfCorrectionBtn.setToolTip(
            "GF Instruments output calibrated ECa values."
        ...
        self.projLabel.setToolTip(
            'Project columns "Latitude" and "Longitude" to specified coordinate system (CRS)'
        )
        ...
        self.projEdit.setToolTip(
            "Type the CRS projection and then select from the options\nDefault is British National Grid / OSGB 1936"
        )
        ...
        self.projBtn.setToolTip(
            "Convert NMEA/DMS string or decimal degree to selected coordinate system (CRS) - select a CRS first"
        )
        ...
        def keepApplyBtnFunc():
            ...
            self.writeLog(
                "k.filterRange(vmin={:s}, vmax={:s})".format(str(vmin), str(vmax))
            )
            self.replot()
        ...
        def rollingBtnFunc():
            window = (
                int(self.rollingEdit.text()) if self.rollingEdit.text() != "" else None
            )
        ...
        self.ptsKillerBtn.setToolTip(
            "Click on points in the figure then click to delete them."
        )
        ...
        def applyBtnFunc():
            self.showParams["vmin"] = (
                float(self.vminEdit.text()) if self.vminEdit.text() != "" else None
            )
            self.showParams["vmax"] = (
                float(self.vmaxEdit.text()) if self.vmaxEdit.text() != "" else None
            )
            self.replot()
        ...
        def expPsMap():
            ...
            fname, _ = QFileDialog.getSaveFileName(
                importTab, "Export raster map", self.datadir, "TIFF (*.tif)"
            )
            if fname != "":
                self.setProjection()
        ...
        self.psMapExpBtn.setToolTip(
            "Export a georeferenced TIFF file to directly be imported in GIS software.\n"
        ...
        def ertImportBtnFunc():
            fname, _ = QFileDialog.getOpenFileName(
                importTab, "Select data file", self.lastdir, "*.dat"
            )
            if fname != "":
                self.lastdir = os.path.dirname(fname)
        ...
        self.meshTypeBtn.setToolTip(
            "Select whether ERT model used is quadrilateral or triangular"
        )
        ...
        self.gfCalibCalibCombo.setToolTip(
            "Select GF calibration used if any."
        ...
        def fitCalibBtnFunc():
            ...
            if self.meshTypeBtn.currentText() == "Triangular ERT Mesh":
                self.meshType = "tri"
            forwardModel = self.forwardCalibCombo.itemText(
                self.forwardCalibCombo.currentIndex()
            )
            calib = (
                self.gfCalibCalibCombo.currentText()
                if self.gfCalibCalibCombo.currentText() != "None"
                else None
            )
            self.mwCalib.setCallback(self.problem.calibrate)
            self.mwCalib.replot(
                fnameECa=self.fnameECa,
                fnameEC=self.fnameEC,
                fnameResMod=self.fnameResMod,
                calib=calib,
                meshType=self.meshType,
                forwardModel=forwardModel,
            )
            self.writeLog(
                'k.calibrate(fnameECa="{:s}", fnameEC="{:s}",'
                'fnameResMod="{:s}", calib="{:s}", meshType="{:s}",'
                'forwardModel="{:s}")'.format(
                    self.fnameECa,
                    str(self.fnameEC),
                    str(self.fnameResMod),
                    str(calib),
                    str(self.meshType),
                    forwardModel,
                )
        ...
        def applyCalibBtnFunc():
            ...
            self.writeLog(
                'k.calibrate(fnameECa="{:s}", fnameEC="{:s}",'
                'fnameResMod="{:s}", calib="{:s}", meshType="{:s}",'
                'forwardModel="{:s}", apply=True)'.format(
                    self.fnameECa,
                    str(self.fnameEC),
                    str(self.fnameResMod),
                    str(calib),
        ...
        self.errLabel = QLabel(
            "EXPERIMENTAL: this tab helps to fit an "
        ...
        def fitErrBtnFunc():
            ...
            self.writeLog(
                'k.crossOverPointsError(index={:d}, coil="{:s}")'.format(index, coil)
            )
            self.mwErrMap.setCallback(self.problem.plotCrossOverMap)
            self.mwErrMap.replot(index=index, coil=coil)
            self.writeLog(
                'k.plotCrossOverMap(index={:d}, coil="{:s}")'.format(index, coil)
            )
        ...
        class ModelTable(QTableWidget):
            ...
            def delRow(self):
                depths0, conds0, fixedDepths, fixedConds = self.getTable()
                self.setTable(
                    depths0[:-1], conds0[:-1], fixedDepths[:-1], fixedConds[:-1]
                )
        ...
        def importModelBtnFunc():
            self._dialog = QFileDialog()
            fname, _ = self._dialog.getOpenFileName(
                settingsTab, "Select data file", self.lastdir, "*.csv"
            )
            if fname != "":
                # try:
                self.lastdir = os.path.dirname(fname)
        ...
        self.importModelBtn.setToolTip(
            "File needs to be .csv with layer1, layer2, depth1, ... columns.\n"
            "layerX contains EC in mS/m and depthX contains depth of the bottom\n"
            "of the layer in meters (positively defined)"
        )
        ...
        def createModelBtnFunc():
            ...
            thick = (
                ...
                if self.thicknessEdit.text() != ""
                else 0.5
            )
            ecstart = (
                float(self.startingEdit.text())
                if self.startingEdit.text() != ""
                else 20
            )
        ...
        self.alphaEdit.setToolTip(
            "Vertical smoothing between layers from the same profiles.\n"
        ...
        self.betaEdit.setToolTip(
            "Lateral smoothing between contiguous profiles.\n 0 means no lateral smoothing."
        )
        ...
        self.gammaEdit.setToolTip(
            "Smoothing between the first survey and other surveys."
        )
        ...
        if frozen != "not":
            self.parallelCheck.setEnabled(
                False
            )  # TODO on available on unfrozen version
        ...
        self.annSampleEdit.setToolTip(
            "Number of synthetic samples for training the model."
        )
        ...
        self.annNoiseEdit.setToolTip(
            "Noise in percent to apply on synthetic data for training the network."
        )
        ...
        def invertBtnFunc():
            ...
            self.writeLog(
                "k.setInit(depths0={:s}, conds0={:s}, fixedDepths={:s}, fixedConds={:s})".format(
                    str(list(depths0)),
                    str(list(conds0)),
                    str(list(fixedDepths)),
                    str(list(fixedConds)),
                )
            )
            regularization = self.lCombo.itemText(self.lCombo.currentIndex())
            alpha = (
                float(self.alphaEdit.text()) if self.alphaEdit.text() != "" else 0.07
            )
            ...
            nsample = (
                int(self.annSampleEdit.text())
                if self.annSampleEdit.text() != ""
                else 100
            )
            noise = (
                float(self.annNoiseEdit.text()) if self.annNoiseEdit.text() != "" else 0
            )
            ...
            for i in range(len(depths) - 1):
                if self.icustomModel | (np.sum(~fixedConds) > 0):
                    ...
                    self.sliceCombo.addItem(
                        "{:.2f}m - {:.2f}m".format(depths[i], depths[i + 1])
                    )
            ...
            if forwardModel == "CS (fast)":
                ...
                self.writeLog(
                    'k.invert(forwardModel="{:s}", alpha={:s}, '
        ...
        def applyInvBtnFunc():
            vmin = (
                float(self.vminInvEdit.text())
                if self.vminInvEdit.text() != ""
                else None
            )
            vmax = (
                float(self.vmaxInvEdit.text())
                if self.vmaxInvEdit.text() != ""
                else None
            )
        ...
        def saveInvDataBtnFunc():
            fdir = QFileDialog.getExistingDirectory(
                invTab, "Choose directory where to save the files"
            )
            if fdir != "":
        ...
        def applyInvMapBtnFunc():
            vmin = (
                float(self.vminInvMapEdit.text())
                if self.vminInvMapEdit.text() != ""
                else None
            )
            vmax = (
                float(self.vmaxInvMapEdit.text())
                if self.vmaxInvMapEdit.text() != ""
                else None
            )
        ...
        def expInvMap():
            ...
            if fname != "":
                self.setProjection()
                self.problem.saveSlice(
                    fname=fname,
                    islice=self.sliceCombo.currentIndex(),
                    cmap=self.cmapInvMapCombo.currentText(),
                )
                self.writeLog(
                    'k.saveSlice(fname="{:s}", islice={:d}, cmap="{:s}"'.format(
                        fname,
                        self.sliceCombo.currentIndex(),
                        self.cmapInvMapCombo.currentText(),
                    )
                )
                self.infoDump("File saved in " + fname)

        self.invMapExpBtn = QPushButton("Export GIS")
        self.invMapExpBtn.setToolTip(
            "Export a georeferenced TIFF file to directly be imported in GIS software.\n"
        ...
        def pvapplyBtnFunc():
            vmin = (
                float(self.vminInv3dEdit.text())
                if self.vminInv3dEdit.text() != ""
                else None
            )
            vmax = (
                float(self.vmaxInv3dEdit.text())
                if self.vmaxInv3dEdit.text() != ""
                else None
            )
            self.showInv3dParams["vmin"] = vmin
            self.showInv3dParams["vmax"] = vmax
            threshMin = (
                float(self.pvthreshMin.text())
                if self.pvthreshMin.text() != ""
                else None
            )
            threshMax = (
                float(self.pvthreshMax.text())
        ...
        def saveVtkBtnFunc():
            fname, _ = QFileDialog.getSaveFileName(
                self, "Open File", self.datadir, "VTK (*.vtk)"
            )
            if fname != "":
                self.problem.saveVTK(
                    fname,
                    index=self.showInv3dParams["index"],
...
<p> The below table provides an example of headers containing the details of a device with 3 receiver coils operated at ground level, a frequency of 9000 Hz and with both HCP and PRP coil orientations.</p>
   <p>
        ...
        <img width=800 src="'''
            + resource_path("image/datalayout.png")
            + """"></img>
</p>
...
<p>
        <img width=300 src="'''
            + resource_path("image/calibfile.png")
            + '''"></img>
        <img width=500 src="'''
            + resource_path("image/ec_header.png")
            + """"></img>
</p>
        ...
        <p>
            <img src="'''
            + resource_path("image/smooth-sharp.png")
            + '''" width=600></img>
        </p>
        ...
        <p>
            <img width=250 src="'''
            + resource_path("image/lcurve.png")
            + """"></img>
    ...
    def setProjection(self):
        ...
        try:
            ...
            self.errorDump(
                'CRS projection is not correctly defined - See "Importing" tab'
            )
    ...
    def projBtnFunc(self):
        if self.projEdit.text() == "":
            ...
            self.writeLog(
                'k.convertFromCoord(targetProjection="{:s}")'.format(
                    self.problem.projection
                )
            )
            self.replot()
            # except Exception as e:
    ...
    def replot(self):
        ...
        if self.showCombo.currentText() == "Map":
            ...
            self.writeLog(
                'k.showMap(index={:d}, coil="{:s}", contour={:s},'
                ' vmin={:s}, vmax={:s}, pts={:s}, cmap="{:s}")'.format(
                    index, coil, str(contour), str(vmin), str(vmax), str(pts), cmap
                )
            )
        else:
            self.mwRaw.replot(index=index, coil=coil, vmin=vmin, vmax=vmax, dist=dist)
            if self.showCombo.currentText() == "Dots":
                self.writeLog(
                    'k.show(index={:d}, coil="{:s}",'
                    " vmin={:s}, vmax={:s}, dist={:s})".format(
                        index, coil, str(vmin), str(vmax), str(dist)
                    )
                )
            else:
                self.writeLog(
                    'k.showPseudo(index={:d}, coil="{:s}",'
                    " vmin={:s}, vmax={:s}, dist={:s})".format(
                        index, coil, str(vmin), str(vmax), str(dist)
                    )
                )
    ...
    def processFname(self, fnames, merged=False):
        ...
        coils = [
            "{:s}{:.2f}".format(a.upper(), b)
            for a, b in zip(self.problem.cpos, self.problem.cspacing)
        ]
        if np.sum(np.in1d(coils, ["VCP0.32", "HCP0.32", "VCP1.48", "HCP1.48"])) > 0:
            ...
            self.infoDump(
                "Files well imported. GF instrument suspected. Check if you need to apply the GF correction."
            )
        else:
            self.gfCorrectionBtn.setVisible(False)
            self.gfCalibCombo.setVisible(False)
    ...
    def errorDump(self, text, flag=1):
        ...
        self.errorLabel.setText(
            '<i style="color:' + col + '">[' + timeStamp + "]: " + text + "</i>"
        )
    ...
    def saveData(self):
        fdir = QFileDialog.getExistingDirectory(
            self, "Choose directory where to save the files"
        )
        if fdir != "":
    ...
    def updateChecker(self):  # check for new updates on gitlab
        ...
        try:
            versionSource = urlRequest.urlopen(
                "https://gitlab.com/hkex/emagpy/raw/master/src/version.txt?inline=false"
            )
            versionCheck = versionSource.read().decode()
            version = versionCheck.split()[
                1
            ]  # assuming version number is in 2nd line of version.txt
    ...
    def updateCheckerShow(self, version):
        version_int = int(
            "".join(["{:03d}".format(int(d)) for d in version.split(".")])
        )
        EMagPy_version_int = int(
            "".join(["{:03d}".format(int(d)) for d in EMagPy_version.split(".")])
        )
        ...
        if EMagPy_version_int < version_int:
            ...
            msg.setInformativeText(
                """Please download the latest version of EMagPy at:<p><a href='https://gitlab.com/hkex/emagpy#downloads'>https://gitlab.com/hkex/emagpy</a></p><br>"""
            )
            ...
            if msg.clickedButton() == bttnUpY:
                webbrowser.open(
                    "https://gitlab.com/hkex/emagpy#downloads"
                )  # can add download link, when we have a direct dl link
...
if __name__ == "__main__":
    ...
    app.setStyle("Fusion")
    app.setWindowIcon(
        QIcon(os.path.join(bundle_dir, "logo.png"))
    )  # that's the true app icon
