...
import numpy as np
import pandas as pd
from emagpy.Survey import Survey, idw, griddata
...
from src.emagpy.Survey import clipConcaveHull, tricontourf_clipped
...
class Problem(object):
    ...
    def __init__(self):
        ...
        self.depths0 = np.array(
            [0.5, 1.5]
        )  # initial depths of the bottom of each layer (last one is -inf)
        self.conds0 = []  # initial conductivity for each layer
        self.fixedConds = []
        self.fixedDepths = []
        self.forwardModel = (
            None  # store the forward model choosen for showMisfit and showOne2One
        )
        ...
        self.runningUI = (
            False  # True if run in UI, just change output of parallel stuff
        )
    ...
    def createSurvey(
        self, fname, freq=None, hx=None, targetProjection=None, unit="ppt"
    ):
        ...
        survey = Survey(
            fname, freq=freq, hx=hx, targetProjection=self.projection, unit=unit
        )
    ...
    def createTimeLapseSurvey(self, fnames, targetProjection=None, unit="ppt"):
        ...
        if isinstance(fnames, list):  # it's a list of filename
            ...
            if os.path.isdir(fnames):
                fnames = [
                    os.path.join(fnames, f)
                    for f in np.sort(os.listdir(fnames))
                    if f[0] != "."
                ]
    ...
    def createMergedSurvey(
        self, fnames, method="nearest", how="add", targetProjection=None, unit="ppt"
    ):
        ...
        if len(np.unique(coils)) < len(coils):
            raise Exception(
                "Each survey to be merged should have different coil configurations."
            )
    ...
    def invert(
        ...
        if (
            (method not in mMinimize)
            and (method not in mMCMC)
            and (method not in mOther)
        ):
        ...
        if (forwardModel == "CSgn") | (
            (forwardModel == "CS") & (method == "Gauss-Newton")
        ):
        ...
        if method == "Gauss-Newton":
            if forwardModel == "Q":
                raise ValueError(
                    "Forward model Q can not be used with Gauss-Newton at the moment."
            ...
            if any(self.fixedDepths) is False:
                raise ValueError(
                    "As of the current implementation, all depths should be fixed for Gauss-Newton inversion"
                )
        ...
        if njobs != 1 and beta != 0:
            dump(
                "WARNING: No parallel is possible with lateral smoothing (beta > 0).\n"
            )
            njobs = 1
        ...
        if bnds is not None:
            if len(bnds) == 2 and (
                isinstance(bnds[0], int) or isinstance(bnds[0], float)
            ):
            ...
            if len(bnds) != nparam:
                raise ValueError(
                    "len(bounds) ({:d}) should be the same as "
        ...
        if method == "GPS":  # pragma: no cover
            ...
            if len(self.conds0) > 0:
                ...
                fixedParams = None
            bestConds, bestDepths, bestMisfits, paramSd, paramMin, paramMax = (
                self.gridParamSearch(
                    forwardModel=fmodel,
                    bnds=bnds,
        ...
        if method == "ANN":  # pragma: no cover
            ...
            if bounds is None:  # happen where all depths are fixed
                ...
                dump("bounds = " + str(bounds) + "\n")
            self.buildANN(
                fmodel, bounds, noise=noise, nsample=nsample, dump=dump, iplot=annplot
            )
        ...
        if regularization == "l1":
            ...
            def objfunc(p, app, pn, spn, alpha, beta, gamma, ini0):
                return np.sqrt(
                    np.sum(np.abs(dataMisfit(p, app, ini0))) / len(app)
            ...
            def objfunc(p, app, pn, spn, alpha, beta, gamma, ini0):
                return np.sqrt(
                    np.sum(dataMisfit(p, app, ini0) ** 2) / len(app)
        ...
        if method in mMCMC:
            ...
            class spotpy_setup(object):
                def __init__(
                    self, obsVals, bounds, pn, spn, alpha, beta, gamma, ini0, fmodel
                ):
                    self.params = []
                ...
                def evaluation(
                    self,
                ):  # what the function return when called with the optimal values
        ...
        def solve(obs, pn, spn, alpha, beta, gamma, ini0):
            ...
            if method in mMinimize:  # minimize
                ...
                res = minimize(
                    objfunc,
                    x0,
                    args=(obs, pn, spn, alpha, beta, gamma, ini0),
                    method=method,
                    bounds=bounds,
                    options=options,
                )
        ...
        for i, survey in enumerate(self.surveys):
            ...
            for j in range(nrows):
                ...
                if forwardModel == "Q":
                    obs = np.array(
                        [eca2Q(a * 1e-3, s) for a, s in zip(obs, self.cspacing)]
                    )
                if forwardModel == "QP":
                    obs = np.array(
                        [eca2Q(a * 1e-3, s) for a, s in zip(obs, self.cspacing)]
            ...
            # artifical neural network inversion
                ...
                ibad1 = np.array(
                    [outs[:, l] < bounds[l][0] for l in range(len(bounds))]
                ).any(0)
                ibad2 = np.array(
                    [outs[:, l] > bounds[l][1] for l in range(len(bounds))]
                ).any(0)
                ibad = ibad1 | ibad2
                self.annReplaced = np.sum(ibad)
                if np.sum(ibad) > 0:
                    dump(
                        "WARNING: ANN: {:d} values out of bounds replaced by L-BFGS-G values."
            ...
            if (method == "ANN") or (njobs != 1):
                for j, outt in enumerate(outs):
                    ...
                    rmse[j] = (
                        np.sqrt(
                            np.sum((dataMisfit(out, obs, params[j][-1]) / obs) ** 2)
                            / len(obs)
                        )
                        * 100
                    )
    ...
    def buildANN(
        ...
        if iplot:
            ...
            def plot_history(history):
                ...
                ax.plot(hist["epoch"], hist["mean_absolute_error"], label="Training")
                ax.plot(
                    hist["epoch"], hist["val_mean_absolute_error"], label="Validation"
                )
                ax.legend()
    ...
    def invertGN(self, alpha=0.07, alpha_ref=None, dump=None):
        ...
        for i, survey in enumerate(self.surveys):
            ...
            for j in range(survey.df.shape[0]):
                app = apps[j, :]
                cond = np.ones((len(conds0), 1)) * np.nanmean(
                    app
                ...
                for l in range(
                    ...
                    LHS = np.dot(J.T, J) + alpha * L
                    RHS = np.dot(J.T, d[:, None]) - alpha * np.dot(
                        L, cond
                    )  # minus or plus doesn't matter here ?!
                    if (
                        alpha_ref is not None
                    ):  # constrain the change of the last element of the profile
                        LHS[-1:, -1:] = alpha_ref
                ...
                model[j, :] = out
                rmse[j] = (
                    np.sqrt(np.sum((dataMisfit(out, app) / app) ** 2) / len(app)) * 100
                )
    ...
    def tcorrECa(self):  # pragma: no cover
        ...
        for _, _ in enumerate(self.surveys):
            pass

    def tcorrEC(self):  # pragma: no cover
    ...
    def forward(
        self, forwardModel="CS", coils=None, noise=0.0, models=None, depths=None
    ):
        ...
        if forwardModel in ["CS", "FSlin", "FSeq"]:
            ...
            raise ValueError(
                "Forward model {:s} is not available."
    ...
    def computeSens(self, forwardModel="CS", coils=None, models=None, depths=None):
    ...
    def show(self, index=0, coil="all", ax=None, vmin=None, vmax=None, dist=True):
    ...
    def showDist(self, index=0, coil=None, nbins=20, vmin=None, vmax=None, ax=None):
        ...
        """
        self.surveys[index].showDist(
            coil=coil, nbins=nbins, vmin=vmin, vmax=vmax, ax=ax
        )
    ...
    def showPseudo(
        self, index=0, ax=None, vmin=None, vmax=None, dist=True, cmap="viridis_r"
    ):
        """Show the raw data of the survey.
        ...
        out = ksens.computeSens(
            forwardModel="CS", coils=self.coils, models=[cond], depths=[depth[None, :]]
        )
        sens = out[0].squeeze(-1)  # depth x coils
        mdepths = np.r_[
            depth[0] / 2,
            depth[:-1] + np.diff(depth) / 2,
    ...
    def saveSlice(
        ...
        try:
            ...
            raise ImportError(
                "Rasterio is needed to save georeferenced .tif file. Install it using 'pip install rasterio'"
            )
    ...
    def saveData(self, outputdir):
        ...
        for i, survey in enumerate(self.surveys):
            ...
            if i < len(self.models):  # add inverted data if available
                lcol = [
                    "layer{:d}".format(a + 1) for a in range(self.models[0].shape[1])
                ]
                dcol = [
                    "depth{:d}".format(a + 1) for a in range(self.depths[0].shape[1])
                ]
    ...
    def interpData(self, surveyIndex=-1, method="nearest"):
        ...
        if surveyIndex == -1:
            locs = np.vstack(
                [s.df[["x", "y", "elevation"]].values for s in self.surveys]
            )
        else:
    ...
    def gridData(
        ...
        for survey in self.surveys:
            survey.gridData(
                nx=nx, ny=ny, method=method, xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax
            )
        ...
        if conds0 is None:
            ...
            if len(conds0.shape) == 2:  # matrix
                if conds0.shape[1] != ndepth + 1:
                    raise ValueError(
                        "Number of layers shoud be exactly equal to number of depths + 1."
                    )
                for s in self.surveys:
    ...
    def showResults(
        ...
        if overlay:  # uncertainty overlay
            ...
            if contour is True:
                ...
                yc = np.r_[
                    np.zeros(nsample), centroid[:, 1], -np.ones(nsample) * maxDepth
                ]
                zc = np.c_[zu[:, 0], zu, zu[:, -1]]
                cax = ax.tricontourf(xc, yc, zc.flatten("F"), cmap=acmap, extend="both")
            else:
        ...
        def format_coord(i, j):
            ...
            if col < sig.shape[0]:
                ...
                return "x={0:.4f}, y={1:.4f}, value={2:.4f}".format(
                    col, row, sig[col, row]
                )
            else:
                return ""
    ...
    def saveVTK(self, fname, index=0, maxDepth=None, elev=False):
        ...
        try:
            ...
            threshold = np.max(
                [
                    (np.min(triang.x) - np.max(triang.x)) / 10,
            ...
            print(
                "concave-hull not installed, will use normal convex hull. Install it with `pip install concave-hull`"
            )
            pass
        ...
        with open(fname, "w") as f:
            ...
            for i in range(nelem):
                f.write(
                    "{:d}\t{:s}\n".format(
                        6, " ".join(conMatrix[i, :].astype(str))  # number of vertices
                    )
                )
    ...
    def setModels(self, depths, models):
        ...
        for m, d in zip(models, depths):
            if m.shape[1] != d.shape[1] + 1:
                raise ValueError(
                    "Number of layer should be equal to number of depths + 1 exactly."
                )
            if m.shape[0] != d.shape[0]:
                raise ValueError(
                    "Number of position in layers and depths array should match."
                )
        self.depths = depths
        self.models = models
    ...
    def lcurve(self, isurvey=0, irow=0, alphas=None, ax=None):
        ...
        alphas : list or array-like, optional
        ...
        def objfunc(p, app, alpha):
            return np.sqrt(
                np.sum(dataMisfit(p, app) ** 2) / len(app)
    ...
    def calibrate(
        ...
        if (
            survey.freqs[0] is None
        ):  # fallback in case the use doesn't specify the frequency in the headers
            try:
                survey.freqs = np.ones(len(survey.freqs)) * survey.freqs[0]
                print(
                    "EMI frequency not specified in headers, will use the one from the main data:"
                    + str(survey.freqs[0])
                    + "Hz"
                )
            except:
        ...
        if fnameEC is not None:
            ...
            depths = np.abs(
                [float(a[1:]) for a in dfec.columns if a[0] == "d"]
            )  # those are the depths of at mid layer
            depths = depths[:-1] + np.diff(
                depths
            )  # those are depths of the bottom of the layer
            if calib is not None:
                survey.gfCorrection(calib=calib)

        if survey.df.shape[0] != dfec.shape[0]:
            raise ValueError(
                "input ECa and inputEC should have the same number of rows so the measurements can be paired."
            )
        ...
        for i, coil in enumerate(survey.coils):
            ...
            offsets[i] = intercept
            dump(
                "{:s}: ECa (ERT) = {:.2f} * ECa (EMI) {:+.2f} (R^2={:.2f})".format(
        ...
        if apply:
            if self.calibrated:  # already calibrated
                dump(
                    "Data can only be calibrated once!"
            ...
            for s in self.surveys:
                for i, c in enumerate(self.coils):
                    ...
                    s.df.loc[:, c] = (
                        s.df[c].values + offsets[i] - (1 - slopes[i]) * s.df[c].values
                    )
    ...
    def showSlice(
        ...
        if vmax is None:
            vmax = np.nanmax(z)
        if (
            contour is False or (y == y[0]).all() or (x == x[0]).all()
        ):  # can not contour if they are on a line
            if contour is True:
            ...
            cax = tricontourf_clipped(
                x, y, z, ax=ax, levels=levels, cmap=cmap, extend="both"
            )
            if pts:
                ax.plot(x, y, "k+")
        ax.set_xlabel("x [m]")
    ...
    def computeDOI(self, conds=None, depths=None, nlayers=50):
        ...
        for i in range(len(depths)):  # for each survey
            ...
            # interpolate
            conds2 = np.array(
                [np.interp(dm, mdepths[j, :], conds[i][j, :]) for j in range(n)]
            )
            ...
            imin = np.argmin(
                np.abs(cumS - 0.3), axis=0
            )  # depth closes to 70% cumulative sensitivity
    ...
    def gridParamSearch(
        ...
        bestMod = (
            []
        )  # TODO better to already set the size of these arrays using np.zeros((m,n))*np.nan, and then just assign items. This will make the code faster
        modList = []
        paramMin = []
        paramMax = []
        paramSd = []
        for i in range(
            0, eca.shape[0]
        ):  # TODO this will be slow, but not sure we can speed it up and might still be faster than other solvers
            eca_d = eca[i, :]
            if regularization == "l1":
            ...
            converged = (
                totalMisfit < misfitMax
            )  # TODO I think this can be a boolean array (boolean are faster than int arrays)
            convergedModels = np.hstack(
                (modParams[converged, :][0], totalMisfit[converged][:, None])
            )
            ...
            bestMod.append(
                np.append(
                    modParams[np.where(totalMisfit == np.min(totalMisfit))[0][0], :],
                    np.min(totalMisfit),
                )
            )
