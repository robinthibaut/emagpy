...
if __name__ == "__main__":
    ...
    for file in zf.infolist():
        ...
        percentage = int(extracted_size / uncompress_size * 100)
        progressBar.setValue(percentage)
        if percentage > 50 and percentage < 70:
            splash.showMessage(
                "Copying temp files", Qt.AlignBottom | Qt.AlignCenter, Qt.black
            )
        if percentage >= 70 and percentage < 80:
            splash.showMessage(
                "Checking files", Qt.AlignBottom | Qt.AlignCenter, Qt.black
            )
        if percentage >= 80 and percentage < 90:
            splash.showMessage(
                "Loading PyQt", Qt.AlignBottom | Qt.AlignCenter, Qt.black
            )
        if percentage >= 90 and percentage < 98:
            splash.showMessage("Loading App", Qt.AlignBottom | Qt.AlignCenter, Qt.black)
        if percentage >= 98:
            splash.showMessage(
                "Almost there!", Qt.AlignBottom | Qt.AlignCenter, Qt.black
            )
        app.processEvents()
    ...
    appDir = os.path.join(
        bundle_dir, "ui"
    )  # zip always putting a double dir ... don't know why
    ...
    if OS == "Linux":
        ...
        Popen(
            os.path.join(appDir, "EMagPy.exe"), shell=False, stdout=None, stdin=None
