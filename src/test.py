...
k = Problem()
k.importGF(
    datadir + "cover-crop/coverCropLo.dat", datadir + "cover-crop/coverCropHi.dat"
)
k.importGF(datadir + "cover-crop/coverCropLo.dat")
k.importGF(datadir + "cover-crop/coverCropHi.dat")
k.importGF(
    datadir + "cover-crop/coverCropLo.dat",
...
k = Problem()
k.importGF(
    fnameLo=datadir + "potatoes/potatoesLo.dat",
...
k.createMergedSurvey(
    [datadir + "potatoes/potatoesLo.csv", datadir + "potatoes/potatoesHi.csv"],
    targetProjection="EPSG:27700",
    how="first",
)
...
k.surveys[0].driftCorrection(
    xStation=338379, yStation=405420, radius=5, fit="all", apply=True
)
k.surveys[0].driftCorrection(xStation=338379, yStation=405420, radius=5, fit="all")
...
k.setInit(
    depths0=[0.3, 0.7], fixedDepths=[True, False], fixedConds=[False, True, False]
)
k.invert(
    method="DREAM",
    rep=500,
...
for m in ["L-BFGS-B", "ROPE"]:
    for fm in ["CS", "FSlin", "FSeq", "Q"]:
        ...
        infos.append(
            "{:8s} {:8s} ({:5.2f}s) RMSPE: {:6.2f} %".format(
...
k.calibrate(
    datadir + "calib/dfeca.csv",
    datadir + "calib/dfec.csv",
    forwardModel="CS",
    apply=False,
)
k.calibrate(
...
k.calibrate(
    fnameECa=datadir + "boxford-calib/eca_calibration.csv",
    meshType="quad",
    fnameResMod=datadir + "boxford-calib/quadmesh.dat",
    binInt=4,
)
...
k.showSlice()
k.saveMap(
    fname=datadir + "saprolite/map-idw.tiff",
    coils=[k.coils[0]],
    method="idw",
    color=True,
)
